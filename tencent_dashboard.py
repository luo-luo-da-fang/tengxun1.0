...
with col5:
    color = "#00f2fe" if year_detail["营收同比增速%"] >= 0 else "#ff6b6b"
    st.markdown(f'<div class="metric-card"><div class="metric-label">营收增速</div><div class="metric-value" style="background: linear-gradient(90deg, {color}, {color}); -webkit-background-clip: text; background-clip: text;">{year_detail["营收同比增速%"]}%</div></div>', unsafe_allow_html=True)
with col6:
    color = "#00f2fe" if year_detail["净利润同比增速%"] >= 0 else "#ff6b6b"
    st.markdown(f'<div class="metric-card"><div class="metric-label">净利润增速</div><div class="metric-value" style="background: linear-gradient(90deg, {color}, {color}); -webkit-background-clip: text; background-clip: text;">{year_detail["净利润同比增速%"]}%</div></div>', unsafe_allow_html=True)
with col7:
...
fig_trend.update_layout(
    ...
    yaxis=dict(title="营业收入", title_font=dict(color="#00f2fe"), gridcolor='rgba(255,255,255,0.1)'),
    yaxis2=dict(title="归母净利润", title_font=dict(color="#f093fb"), overlaying="y", side="right", gridcolor='rgba(255,255,255,0.1)'),
...
if len(competitors) > 0:
    ...
    for i, row in comparison_df.iterrows():
        ...
        fig_radar_compare.add_trace(go.Scatterpolar(
            ...
            fill="toself",
            name=row["公司"],
            line=dict(color="#00f2fe" if i == 0 else colors[i-1])
        ))
...
if main_company == "腾讯控股":
    ...
    with c1:
        fig_bar = px.bar(business_trend, x="年份", y="营收", color="业务板块", barmode="group",
                         ...
                         color_discrete_map={"增值服务营收":"#00f2fe","金融科技及企业服务营收":"#f093fb","营销服务营收":"#4facfe"})
    ...
    with c2:
        fig_pie_biz = px.pie(business_now, values="营收", names="业务板块", 
                            ...
                            color_discrete_sequence=["#00f2fe", "#f093fb", "#4facfe"])
    ...
    with map_col2:
        ...
        fig_overseas = px.scatter_geo(
            ...
            color="地区名称",
            color_discrete_map={"东南亚": "#00f2fe", "欧美": "#f093fb", "其他海外地区": "#4facfe"},
            size_max=60
        )
...
fig_grow = px.bar(grow_data, x="年份", y="增速(%)", color="增长类型", barmode="group",
                  title="年度业绩增长幅度对比",
                  color_discrete_map={"营收同比增速%":"#00f2fe", "净利润同比增速%":"#f093fb"})
...
fig_asset = px.area(asset_stack, x="年份", y="金额", color="构成",
                    title="企业资产结构历年变化",
                    color_discrete_map={"负债":"#f093fb", "股东权益":"#00f2fe"})
...
with col2:
    ...
    def generate_qr_code(url):
        ...
        qr.add_data(url)
        qr.make(fit=True)
        img = qr.make_image(fill_color="#00f2fe", back_color="#0f0c29")
        buf = BytesIO()
        img.save(buf, format="PNG")
        buf.seek(0)
        return Image.open(buf)
    
    local_url = "https://tengxun0-j6hkukqw6x5g3mmc8xr7lf.streamlit.app/"
...
if user_input:
    ...
    if "净利润" in user_input:
        ...
        ans = f"{select_year}年营业收入{year_detail['营业收入']:.2f}亿元，同比增速{year_detail['营收同比增速%']}%，业务保持稳健增长。"
        ...
        ans = f"本年度营收增速{year_detail['营收同比增速%']}%，净利润增速{year_detail['净利润同比增速%']}%，增长势头良好。"
        ...
        ans = f"{select_year}年腾讯海外营收{tencent_business_data[tencent_business_data['年份'] == select_year]['海外营收'].iloc[0]:.2f}亿元，主要来自东南亚（300亿元）和欧美（350亿元）市场，国际化进程稳步推进。"
...
st.markdown("""
<div class="footer">
    <h3>🚀 腾讯控股年报综合分析看板 | 科技风专业版</h3>
    <p>数据来源：公司官方财报 | 数据更新至2024年</p>
    <p>© 2026 财务数据分析系统 | 所有数据仅供参考，不构成投资建议</p>
</div>
""", unsafe_allow_html=True)
